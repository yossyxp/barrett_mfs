...
void quantities( double t, double *x1, double *x2, double *r, double *t1, double *t2, double *n1, double *n2, double *T1, double *T2, double *N1, double *N2, double *nu, double *phi, double *kappa, double *beta, double *kappa_gamma ); //x --> t,n,T,N,phi,kappa
...
void velocity( double t, double *x1, double *x2, double *n1, double *n2, double *l, double *phi, double *beta, double *kappa, double *kappa_gamma, double L, double A, double *V, double *W ); // x,n,r,t,ohi,kappa,L --> V,W
...
void pre( double t, double *x1, double *x2, double *T1, double *T2, double *N1, double *N2, double *V, double *W ){
  ...
  // T,N
  quantities(t,x1,x2,l,t1,t2,n1,n2,T1,T2,N1,N2,nu,phi,kappa,beta,kappa_gamma);
  
  // L,A
  measure(t,x1,x2,&L,&A);
  
  // V,W
  velocity(t,x1,x2,n1,n2,l,phi,beta,kappa,kappa_gamma,L,A,V,W);
...
//x --> t,n,T,N,phi,kappa
void quantities( double t, double *x1, double *x2, double *l, double *t1, double *t2, double *n1, double *n2, double *T1, double *T2, double *N1, double *N2, double *nu, double *phi, double *kappa, double *beta, double *kappa_gamma ){
  ...
  for( i = 1; i <= NUM; i++ ){
    ...
    //kappa_gamma[i] = -36 * cos(6 * nu[i]);
    //kappa_gamma[i] = ( beta[i] + ( 0.99 / 35.0 ) * 36 * cos(6 * nu[i]) );
    kappa_gamma[i] = - (sqrt(( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) * ( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] ) * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] )) * ( cos(M_PI/3.0) - eps * eps * sin(M_PI/3.0) ) * ( cos(M_PI/3.0) - eps * eps * sin(M_PI/3.0) )
      + sqrt(( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) * ( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] ) * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] )) * ( cos(2*M_PI/3.0) - eps * eps * sin(2*M_PI/3.0) ) * ( cos(2*M_PI/3.0) - eps * eps * sin(2*M_PI/3.0) )
      + sqrt(( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) * ( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) + eps * eps * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] ) * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] )) * ( cos(M_PI) - eps * eps * sin(M_PI) ) * ( cos(M_PI) - eps * eps * sin(M_PI) )
      + sqrt(( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) * ( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] ) * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] )) * ( sin(M_PI/3.0) + eps * eps * cos(M_PI/3.0) ) * ( sin(M_PI/3.0) + eps * eps * cos(M_PI/3.0) )
      + sqrt(( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) * ( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] ) * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] )) * ( sin(2*M_PI/3.0) + eps * eps * cos(2*M_PI/3.0) ) * ( sin(2*M_PI/3.0) + eps * eps * cos(2*M_PI/3.0) )
			+ sqrt(( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) * ( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) + eps * eps * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] ) * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] )) * ( sin(M_PI) + eps * eps * cos(M_PI) ) * ( sin(M_PI) + eps * eps * cos(M_PI) ) );
    /*
    kappa_gamma[i] =
      -( - ( ( cos(M_PI/3.0)*sin(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*sin(M_PI/3.0)*n2[i] - eps * eps * ( cos(M_PI/3.0)*sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*cos(M_PI/3.0)*n2[i]) ) * ( cos(M_PI/3.0)*sin(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*sin(M_PI/3.0)*n2[i] - eps * eps * ( cos(M_PI/3.0)*sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*cos(M_PI/3.0)*n2[i]) ) / ( sqrt(( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) * ( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] ) * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] )) * (( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) * ( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] ) * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] )) )
	  + ( sin(M_PI/3.0) * sin(M_PI/3.0) + eps * eps * cos(M_PI/3.0) * cos(M_PI/3.0) ) / sqrt(( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) * ( cos(M_PI/3.0)*n1[i] + sin(M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] ) * ( -sin(M_PI/3.0)*n1[i] + cos(M_PI/3.0)*n2[i] )) )
      +
      ( -( cos(2*M_PI/3.0)*sin(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*sin(2*M_PI/3.0)*n2[i] - eps * eps * ( cos(2*M_PI/3.0)*sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*cos(2*M_PI/3.0)*n2[i]) ) * ( cos(2*M_PI/3.0)*sin(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*sin(2*M_PI/3.0)*n2[i] - eps * eps * ( cos(2*M_PI/3.0)*sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*cos(2*M_PI/3.0)*n2[i]) ) / ( sqrt(( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) * ( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] ) * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] )) * (( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) * ( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] ) * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] )) )
	+ ( sin(2*M_PI/3.0) * sin(2*M_PI/3.0) + eps * eps * cos(2*M_PI/3.0) * cos(2*M_PI/3.0) ) / sqrt(( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) * ( cos(2*M_PI/3.0)*n1[i] + sin(2*M_PI/3.0)*n2[i] ) + eps * eps * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] ) * ( -sin(2*M_PI/3.0)*n1[i] + cos(2*M_PI/3.0)*n2[i] )) )
    +
      ( -( cos(M_PI)*sin(M_PI)*n1[i] + sin(M_PI)*sin(M_PI)*n2[i] - eps * eps * ( cos(M_PI)*sin(M_PI)*n1[i] + cos(M_PI)*cos(M_PI)*n2[i]) ) * ( cos(M_PI)*sin(M_PI)*n1[i] + sin(M_PI)*sin(M_PI)*n2[i] - eps * eps * ( cos(M_PI)*sin(M_PI)*n1[i] + cos(M_PI)*cos(M_PI)*n2[i]) ) / ( sqrt(( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) * ( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) + eps * eps * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] ) * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] )) * (( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) * ( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) + eps * eps * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] ) * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] )) )
	+ ( sin(M_PI) * sin(M_PI) + eps * eps * cos(M_PI) * cos(M_PI) ) / sqrt(( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) * ( cos(M_PI)*n1[i] + sin(M_PI)*n2[i] ) + eps * eps * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] ) * ( -sin(M_PI)*n1[i] + cos(M_PI)*n2[i] )) ) );
*/
...
void grad_P( double t, double *x_mid1, double *x_mid2, double *n1, double *n2, double *l, double A, double *P, double *beta, double *grad_u1, double *grad_u2 ){
  ...
  for( i = 1; i <= NUM; i++ ){
    ...
    for( j = 1; j <= 2 * NUM; j++ ){
      ...
      G[i][j] = E(x_mid1[i],x_mid2[i],y1[j],y2[j],z1[j],z2[j]) - rho * ip(H1[i][j],H2[i][j],n1[i],n2[i]) / beta[i];
...
void velocity( double t, double *x1, double *x2, double *n1, double *n2, double *l, double *phi, double *beta, double *kappa, double *kappa_gamma, double L, double A, double *V, double *W  ){
